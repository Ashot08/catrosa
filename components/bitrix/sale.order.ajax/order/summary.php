...
<thead>
	<tr>
		...
		<th style="text-align: right;"><?=GetMessage("SOA_TEMPL_SUM_PRICE")?></th>
	</tr>
</thead>
<tbody>
	<?
		...
		<tr>
			<td style="width: 10%;">
				...
				elseif (IntVal($arBasketItems["DETAIL_PICTURE"]) > 0) {
					...
					echo '<img src="/img/catrosa_default_image.jpg" height="100">';
				}
				?>
			</td>
	...
	<tr>
		<td colspan="3" class="itog"><?=GetMessage("SOA_TEMPL_SUM_SUMMARY")?></td>
	...
	if (doubleval($arResult["DISCOUNT_PRICE"]) > 0)
	{
		?>
		<tr>
			<td colspan="3" class="itog"><?=GetMessage("SOA_TEMPL_SUM_DISCOUNT")?><?if (strLen($arResult["DISCOUNT_PERCENT_FORMATED"])>0):?> (<?echo $arResult["DISCOUNT_PERCENT_FORMATED"];?>)<?endif;?>:</td>
	...
	if(!empty($arResult["arTaxList"]))
	{
		foreach($arResult["arTaxList"] as $val)
		{
			?>
			<tr>
				<td colspan="3" class="itog"><?=$val["NAME"]?> <?=$val["VALUE_FORMATED"]?>:</td>
	...
	if (doubleval($arResult["DELIVERY_PRICE"]) > 0)
	{
		?>
		<tr>
			<td colspan="3" class="itog"><?=GetMessage("SOA_TEMPL_SUM_DELIVERY")?></td>
	...
	if (strlen($arResult["PAYED_FROM_ACCOUNT_FORMATED"]) > 0)
	{
		?>
		<tr>
			<td colspan="3" class="itog"><?=GetMessage("SOA_TEMPL_SUM_PAYED")?></td>
	...
	?>
	<tr class="last">
		<td colspan="3" class="itog"><?=GetMessage("SOA_TEMPL_SUM_IT")?></td>
